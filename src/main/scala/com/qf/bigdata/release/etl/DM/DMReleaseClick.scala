...
object DMReleaseClick {
  ...
  def runJob(ss:SparkSession,day:String): Unit ={
    ...
    // 获取当天日志字段数据
    val clickColumns = DMReleaseColumnsHelper.selectDMReleaseClick()
    ...
    val tmpDF: Dataset[Row] = SparkHelper
      .readTableData(ss,ReleaseConstant.DW_RELEASE_CLICK,clickColumns)
      // 查询条件
    ...
    val cubeRet: DataFrame = tmpDF.groupBy(Columns: _*).agg(
      count("*"),
