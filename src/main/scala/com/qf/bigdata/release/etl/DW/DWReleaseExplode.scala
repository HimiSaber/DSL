...
object DWReleaseExplode {
  ...
  def handleReleaseJob(spark:SparkSession,appName:String,bdp_day:String): Unit ={
    try{
      ...
      // 获取当天日志字段数据
      val cusomerColumns = DWReleaseColumnsHelper.selectDWExplodColumns()
      ...
      val cusomerReleaseCondition =
        (col(s"${ReleaseConstant.DEF_PARTITION}")) === lit(bdp_day) and
          (col(s"${ReleaseConstant.COL_RELEASE_SESSION_STATUS}") ===
            lit(ReleaseStatusEnum.SHOW.getCode) or
            col(s"${ReleaseConstant.COL_RELEASE_SESSION_STATUS}") ===
      ...
      // 目标用户
      // SparkHelper.writeTableData(customerReleaseDF,ReleaseConstant.DW_RELEASE_EXPOSURE,saveMode)
